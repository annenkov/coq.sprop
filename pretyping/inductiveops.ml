...
let make_case_or_project env sigma indf ci pred c branches =
  ...
  | Some ps ->
     ...
     let () =
       let _, _, _, t = destLambda sigma pred in
     ...
     let n, subst =
       List.fold_right
	     ...
	     (i + 1, t :: subst)
          | LocalDef (na, _, b, t) -> (i, Vars.substl subst b :: subst))
	 ctx (0, [])
...
let make_arity_signature env sigma dep indf =
  ...
  if dep then
    ...
    Namegen.name_context env sigma
      ((LocalAssum (Anonymous,r,EConstr.of_constr (build_dependent_inductive env indf)))::arsign)
...
let is_predicate_explicitly_dep env sigma pred arsign =
  let rec srec env pval arsign =
    ...
    match EConstr.kind sigma pv', arsign with
      | Lambda (na,r,t,b), (LocalAssum _)::arsign ->
          srec (push_rel_assum (na, r, t) env) b arsign
      | Lambda (na,_,_,t), _ ->
...
let rec instantiate_universes env evdref scl is = function
  ...
  | d::sign, None::exp ->
      d :: instantiate_universes env evdref scl is (sign, exp)
  | (LocalAssum (na,_,ty))::sign, Some l::exp ->
      ...
      let s =
        ...
        if univ_level_mem l is then
          ...
          let evm = Evd.set_leq_sort env evm s (Sorts.sort_of_univ u) in
            evdref := evm; s
      in
      (LocalAssum (na,Sorts.Relevant,mkArity(ctx,s))) :: instantiate_universes env evdref scl is (sign, exp)
