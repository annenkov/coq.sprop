...
let generate_type evd g_to_f f graph i =
  ...
  (*i The [pre_context]  is the defined to be the context corresponding to
    ...
    i*)
  let pre_ctxt =
    LocalAssum (Name res_id, Sorts.Relevant, lift 1 res_type) ::
    LocalDef (Name fv_id, Sorts.Relevant, mkApp (f,args_as_rels), res_type) :: fun_ctxt
  in
  (*i and we can return the solution depending on which lemma type we are defining i*)
  if g_to_f
  then LocalAssum (Anonymous,Sorts.Relevant,graph_applied)::pre_ctxt,(lift 1 res_eq_f_of_args),graph
  else LocalAssum (Anonymous,Sorts.Relevant,res_eq_f_of_args)::pre_ctxt,(lift 1 graph_applied),graph
        ...
        (fun ((_,(ctxt,concl))) ->
           ...
           | hres::res::decl::ctxt ->
             let res = EConstr.it_mkLambda_or_LetIn
                 ...
                 (LocalAssum (RelDecl.get_name decl,
                              RelDecl.get_relevance decl, RelDecl.get_type decl) :: ctxt)
             in
             res)
        lemmas_types_infos
    in
...
let generalize_dependent_of x hyp g =
  ...
  tclMAP
    (function
       | LocalAssum (id,_,t) when not (Id.equal id hyp) &&
