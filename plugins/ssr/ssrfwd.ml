...
let ssrsettac ist id ((_, (pat, pty)), (_, occ)) gl =
  ...
  let cl' = EConstr.mkLetIn (Name id, Sorts.Relevant, c, cty, cl) in
...
let destProd_or_LetIn sigma c =
  ...
  | Prod (n,r,ty,c) -> RelDecl.LocalAssum (n, r, ty), c
  | LetIn (n,r,bo,ty,c) -> RelDecl.LocalDef (n, r, bo, ty), c
  | _ -> raise DestKO
...
let wlogtac ist (((clr0, pats),_),_) (gens, ((_, ct))) hint suff ghave gl =
  ...
  let c, args, ct, gl =
    ...
    let rec var2rel c g s = match EConstr.kind sigma c, g with
      | Prod(Anonymous,r,_,c), [] -> EConstr.mkProd(Anonymous, r, EConstr.Vars.subst_vars s ct, c)
      | Sort _, [] -> EConstr.Vars.subst_vars s ct
      | LetIn(Name id as n,r,b,ty,c), _::g -> EConstr.mkLetIn (n,r,b,ty,var2rel c g (id::s))
      | Prod(Name id as n,r,ty,c), _::g -> EConstr.mkProd (n,r,ty,var2rel c g (id::s))
    ...
    let rec pired c = function
      ...
      | t::ts as args -> match EConstr.kind sigma c with
         | Prod(_,_,_,c) -> pired (EConstr.Vars.subst1 t c) ts
         | LetIn(id,r,b,ty,c) -> EConstr.mkLetIn (id,r,b,ty,pired c args)
