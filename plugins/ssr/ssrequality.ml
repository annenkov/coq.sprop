...
let rec strip_prod_assum c = match Constr.kind c with
  | Prod (_, _, _, c') -> strip_prod_assum c'
  | LetIn (_, _, v, _, c') -> strip_prod_assum (subst1 v c)
...
(*   ppdebug(lazy(str"sigma@pirrel_rewrite=" ++ pr_evar_map None sigma)); *)
  ...
  let sigma, p = 
    ...
    (sigma, ev)
  in
  let pred = EConstr.mkNamedLambda pattern_id Sorts.Relevant rdx_ty pred in
  let elim, gl = 
  ...
  with _ -> 
    ...
    | App (hd, args) -> 
        ...
        let names = let rec aux t = function 0 -> [] | n ->
          ...
          | ProdType (name, _, _, t) -> name :: aux t (n-1)
...
(*         ppdebug(lazy(str"sigma@rwcltac=" ++ pr_evar_map None (fst sr))); *)
  ...
  let cvtac, rwtac, gl =
    if EConstr.Vars.closed0 (project gl) r' then 
      ...
      | _ -> 
          let cl' = EConstr.mkApp (EConstr.mkNamedLambda pattern_id Sorts.Relevant rdxt cl, [|rdx|]) in
      ...
      let cl' = EConstr.mkNamedProd rule_id Sorts.Relevant (EConstr.it_mkProd_or_LetIn r3t dc) (EConstr.Vars.lift 1 cl) in
      let cl'' = EConstr.mkNamedProd pattern_id Sorts.Relevant rdxt cl' in
  ...
  let cvtac' _ =
    ...
    | PRtype_error ->
      ...
      then errorstrm Pp.(str "Rewriting impacts evars")
        ...
        ++ pr_constr_env (pf_env gl) (project gl) (Term.mkNamedLambda pattern_id Sorts.Relevant (EConstr.Unsafe.to_constr rdxt) (EConstr.Unsafe.to_constr cl)))
