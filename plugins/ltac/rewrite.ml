...
module GlobalBindings (M : sig
  ...
  let build_signature evars env m (cstrs : (types * types option) option list)
	...
	match EConstr.kind (goalevars evars) t, l with
	    ...
	    let evars, newarg = app_poly env evars respectful [| ty ; b' ; relty ; arg |] in
              evars, mkProd(na, r, ty, b), newarg, (ty, Some relty) :: cstrs
	  else
	    let (evars, b, arg, cstrs) =
              aux (push_rel (LocalAssum (na, r, ty)) env) evars b cstrs
	    in
            let ty = Reductionops.nf_betaiota env (goalevars evars) ty in
            let pred = mkLambda (na, r, ty, b) in
            let liftarg = mkLambda (na, r, ty, arg) in
            let evars, arg' = app_poly env evars forall_relation [| ty ; pred ; liftarg |] in
              if Option.is_empty obj then evars, mkProd(na, r, ty, b), arg', (ty, None) :: cstrs
  ...
  let unfold_impl sigma t =
    ...
    | App (arrow, [| a; b |])(*  when eq_constr arrow (Lazy.force impl) *) ->
      mkProd (Anonymous, Sorts.Relevant, a, lift 1 b)
    | _ -> assert false
  ...
  let unfold_all sigma t =
    ...
    | App (id, [| a; b |]) (* when eq_constr id (Lazy.force coq_all) *) ->
      ...
      | Lambda (n, r, ty, b) -> mkProd (n, r, ty, b)
      | _ -> assert false)
    | _ -> assert false
  ...
  let arrow_morphism env evd ta tb a b =
	...
	(* (evd, mkProd (Anonymous, a, b)), (fun x -> x) *)
        ...
        (app_poly env evd coq_all [| a; mkLambda (Anonymous, Sorts.Relevant, a, lift 1 b) |]), unfold_forall
  ...
  let pointwise_or_dep_relation env evd n t car rel =
    if noccurn (goalevars evd) 1 car && noccurn (goalevars evd) 1 rel then
      ...
      app_poly env evd forall_relation
        [| t; mkLambda (n, Sorts.Relevant, t, car); mkLambda (n, Sorts.Relevant, t, rel) |]
	  ...
	  if noccurn sigma 1 b then
	    ...
	    let evars, rb = aux evars env b' (pred n) in
	      app_poly env evars pointwise_relation [| ty; b'; rb |]
            ...
            let evars, rb = aux evars (push_rel (LocalAssum (na, r, ty)) env) b (pred n) in
	      app_poly env evars forall_relation
                [| ty; mkLambda (na, r, ty, b); mkLambda (na, r, ty, rb) |]
	| _ -> raise Not_found
    in 
...
let rec decompose_app_rel env evd t = 
  ...
  | App (f, [|arg|]) ->
    ...
    let f'' = mkLambda (Name default_dependent_ident, r, ty,
      mkLambda (Name (Id.of_string "y"), r, lift 1 ty,
...
let decompose_applied_relation env sigma (c,l) =
  ...
  in
	...
	let ctx,t' = Reductionops.splay_prod env sigma ctype in (* Search for underlying eq *)
        match find_rel (it_mkProd_or_LetIn t' (List.map (fun (n,r,t) -> LocalAssum (n, r, t)) ctx)) with
	| Some c -> c
...
let resolve_morphism env avoid oldt m ?(fnewt=fun x -> x) args args' (b,cstr) evars =
  let evars, morph_instance, proj, sigargs, m', args, args' =
	...
	if b then PropGlobal.do_subrelation, PropGlobal.apply_subrelation 
      ...
      in
	EConstr.push_named
          (LocalDef (Id.of_string "do_subrelation", Sorts.Relevant,
  ...
  let prf = 
	...
	let prf =
	  e_app_poly env evars coq_f_equal
		[| r.rew_car; ty;
                   mkLambda (Anonymous, Sorts.Relevant, r.rew_car, c);
...
let subterm all flags (s : 'a pure_strategy) : 'a pure_strategy =
  let rec aux { state ; env ; unfresh ;
    ...
    let cstr' = Option.map (fun c -> (ty, Some c)) cstr in
      ...
      | Prod (n, r, x, b) when noccurn (goalevars evars) 1 b ->
      ...
      | Prod (n, r, dom, codom) ->
          let lam = mkLambda (n, r, dom, codom) in
 ...
 *)
      ...
      | Lambda (n, relevance, t, b) when flags.under_lambdas ->
        let n' = Nameops.Name.map (fun id -> Tactics.fresh_id_in_env unfresh id env) n in
        let open Context.Rel.Declaration in
        let env' = EConstr.push_rel (LocalAssum (n', relevance, t)) env in
	...
	let res = 
	  ...
	  | Success r ->
		...
		let evars, rel = point env r.rew_evars n' t r.rew_car rel in
                let prf = mkLambda (n', relevance, t, prf) in
		  { r with rew_prf = RewPrf (rel, prf); rew_evars = evars }
	      | x -> r
	    in
	      Success { r with
                rew_car = mkProd (n, relevance, t, r.rew_car);
                rew_from = mkLambda(n, relevance, t, r.rew_from);
                rew_to = mkLambda (n, relevance, t, r.rew_to) }
	  | Fail | Identity -> b'
	in state, res
	  ...
	  let term =
	    ...
	    | Some (t, ty) ->
              ...
              let ty = Reductionops.nf_evar evars' ty in
                mkApp (mkLambda (Name (Id.of_string "lemma"), Sorts.Relevant, ty, p), [| t |])
	  in
...
let assert_replacing id newt tac = 
  let prf = Proofview.Goal.enter begin fun gl ->
    ...
    | [] -> assert false
    | d :: rem -> insert_dependent env sigma (LocalAssum (NamedDecl.get_id d, Sorts.Relevant, newt)) [] after @ rem
    in
...
let cl_rewrite_clause_newtac ?abs ?origsigma ~progress strat clause =
  ...
  let treat sigma res = 
	...
	| Some id, None ->
            Proofview.Unsafe.tclEVARS undef <*>
            convert_hyp_no_check (LocalAssum (id, Sorts.Relevant, newt)) <*>
            beta_hyp id
	| None, Some p ->
            Proofview.Unsafe.tclEVARS undef <*>
    ...
    Proofview.tclORELSE
      begin
        try
          ...
          let car = pi3 (List.hd (fst (Reductionops.splay_prod env sigma t))) in
