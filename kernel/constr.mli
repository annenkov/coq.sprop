...
*)
type rec_declaration = Name.t array * Sorts.relevance array * types array * constr array
...
type ('constr, 'types, 'sort, 'univs) kind_of_term =
  ...
  | Prod      of Name.t * Sorts.relevance * 'types * 'types             (** Concrete syntax ["forall A:B,C"] is represented as [Prod (A,B,C)]. *)
  | Lambda    of Name.t * Sorts.relevance * 'types * 'constr            (** Concrete syntax ["fun A:B => C"] is represented as [Lambda (A,B,C)].  *)
  | LetIn     of Name.t * Sorts.relevance * 'constr * 'types * 'constr  (** Concrete syntax ["let A:C := B in D"] is represented as [LetIn (A,B,C,D)]. *)
