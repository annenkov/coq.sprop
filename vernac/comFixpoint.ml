...
let prepare_recursive_declaration fixnames fixrs fixtypes fixdefs =
  ...
  (Array.of_list names, Array.of_list fixrs, Array.of_list fixtypes, Array.of_list defs)
    ...
    List.fold_left2
      (fun (sigma, env') id t ->
         if program_mode then
           ...
           in
           sigma, LocalAssum (id,Sorts.Relevant,fixprot) :: env'
         else sigma, LocalAssum (id,Sorts.Relevant,t) :: env')
