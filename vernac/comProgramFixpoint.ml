...
let mkSubset sigma name typ prop =
  ...
  sigma, mkApp (app_h, [| typ; mkLambda (name, Sorts.Relevant, typ, prop) |])
...
let rec telescope sigma l =
  ...
  | [] -> assert false
  | [LocalAssum (n, r, t)] ->
    sigma, t, [LocalDef (n, r, mkRel 1, t)], mkRel 1
  | LocalAssum (n, r, t) :: tl ->
      let sigma, ty, tys, (k, constr) =
        List.fold_left
          (fun (sigma, ty, tys, (k, constr)) decl ->
             let t = RelDecl.get_type decl in
             let rt = RelDecl.get_relevance decl in
            let pred = mkLambda (RelDecl.get_name decl, rt, t, ty) in
      ...
      let sigma, last, subst = List.fold_right2
        (fun pred decl (sigma, prev, subst) ->
          ...
          let proj2 = applist (p2, [t; pred; prev]) in
            (sigma, lift 1 proj2, LocalDef (get_name decl, Sorts.Relevant, proj1, t) :: subst))
        (List.rev tys) tl (sigma, mkRel 1, [])
      in sigma, ty, (LocalDef (n, Sorts.Relevant, last, t) :: subst), constr

  | LocalDef (n, r, b, t) :: tl ->
    let sigma, ty, subst, term = telescope sigma tl in
    sigma, ty, (LocalDef (n, r, b, t) :: subst), lift 1 term
...
let build_wellfounded (recname,pl,n,bl,arityc,body) poly r measure notation =
  ...
  let arg = LocalAssum (Name argname, Sorts.Relevant, argtyp) in
    ...
    in
      try
        let ctx, ar = Reductionops.splay_prod_n env sigma 2 relty in
          ...
          | [LocalAssum (_,_,t); LocalAssum (_,_,u)], Sort s
  ...
  let wfarg sigma len =
    ...
    sigma, LocalAssum (Name argid', Sorts.Relevant, ss_term)
  in
  let sigma, intern_bl =
  ...
  let wfargpred = mkLambda (Name argid', Sorts.Relevant, argtyp, wf_rel_fun (mkRel 1) (mkRel 3)) in
  ...
  let intern_fun_binder = LocalAssum (Name (add_suffix recname "'"), Sorts.Relevant, intern_fun_arity_prod) in
  let sigma, curry_fun =
    let wfpred = mkLambda (Name argid', Sorts.Relevant, argtyp, wf_rel_fun (mkRel 1) (mkRel (2 * len + 4))) in
    let sigma, intro = Evarutil.new_global sigma (delayed_force build_sigma).Coqlib.intro in
    let arg = mkApp (intro, [| argtyp; wfpred; lift 1 make; mkRel 1 |]) in
    let app = mkApp (mkRel (2 * len + 2 (* recproof + orig binders + current binders *)), [| arg |]) in
    let rcurry = mkApp (rel, [| measure; lift len measure |]) in
    let lam = LocalAssum (Name (Id.of_string "recproof"), Sorts.Relevant, rcurry) in
    let body = it_mkLambda_or_LetIn app (lam :: binders_rel) in
    let ty = it_mkProd_or_LetIn (lift 1 top_arity) (lam :: binders_rel) in
    sigma, LocalDef (Name recname, Sorts.Relevant, body, ty)
  in
  let fun_bl = intern_fun_binder :: [arg] in
  let lift_lets = lift_rel_context 1 letbinders in
  let sigma, intern_body =
    let ctx = LocalAssum (Name recname, Sorts.Relevant, get_type curry_fun) :: binders_rel in
  ...
  let prop = mkLambda (Name argname, Sorts.Relevant, argtyp, top_arity_let) in
